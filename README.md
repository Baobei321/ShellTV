...
2. 根据[基础语法]()和[补充语法定义](#️-对源配置文件语法的补充定义)，对模板进行修改，包括，删除、排序或拼接等（注，修改过程中要确保其中的索引文件地址正确）
3. 将修改后的配置文件和相关的其它文件上传到机顶盒：打开[配置界面]()、[用电脑或手机连接]()、[创建一个文件夹(如shelltv)]()、[将直播配置文件上传至该文件夹]()、[在APP源配置界面中填写正确地址（如，）]()、确定保存并退出。
