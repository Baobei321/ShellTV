...
## 🛠️ 对源配置文件语法扩展
为改进用户体验，**电视壳子**对TvBox项目的[源配置文件的语法定义](./tvbox源配置文件基本语法.md)进行了扩展，如果你是进阶玩家（致力于定制属于自己的配置文件），可以使用这些扩展定义，并配合**电视壳子**使用，以获取更好的使用体验（如，避免收藏与历史记录中出现大量无意义的海报，或无法屏蔽已失效的阿里资源）。这些语法扩展多用于网盘搜索类、原数据无人维护的源。
