...
- 对数据源文件语法增加一些定义，以更好处理影片元数据缺失问题（如海报等）Your environment does not support JCEF, cannot use Markdown Editor
- 其它一些功能设置
...
## 🛠️ 对源配置文件的补充语法定义
为改进用户体验，**电视壳子**对TvBox项目的源配置文件补充了一些语法定义，如果你是进阶玩家（致力于定制属于自己的配置文件），可以使用这些定义，并配合**电视壳子**使用，可以达到更好的用户体验（如避免收藏与历史记录中出现一堆无意义的海报）。这些补充定义多用于网盘搜索类、无人维护原数据的源。

以下是这些语法语义描述：

| 属性       | 描述                     | 缺省值   |
|------------|--------------------------|--------|
| hasPoster| 该源提供海报  | 1，提供 |
| hasMeaningfulTitle| 该源提供准确标题| 1，有关 |
| hasTags       | 该源提供影片的原数据      | 1，提供|
...
## 📥 捐赠
由于《**电视壳子**》的部分功能需要服务器资源支持，为覆盖基本开销，我们接受捐赠。
