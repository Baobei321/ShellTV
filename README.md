...
## 🛠️ 对源配置文件语法的补充定义
为改进用户体验，**电视壳子**对TvBox项目的[源配置文件的语法定义](./tvbox源配置文件基本语法.md)进行了扩展，如果你是进阶玩家（致力于定制属于自己的配置文件），可以使用这些扩展定义，并配合电视壳子使用，以获取更好的使用体验（如，避免收藏与历史记录中出现大量无意义的海报）。这些语法扩展多用于网盘搜索类、原数据无人维护的源。
...
#### 入门玩家
1. 百度或Github 搜索配置文件，复制URL地址，关键词：TVBox配置（最好先在电脑上试一下配置文件是否能正常下载）。
...
## 🎉 捐赠
由于**电视壳子**部分特色功能依赖于服务器资源，为覆盖基本开销，我们接受捐赠。您的捐赠将用于云服务器、域名等费用，并支持我们走得更远。
方式：在应用首页，点击捐赠按钮
